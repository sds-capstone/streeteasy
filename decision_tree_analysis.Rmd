...
#in total we have 42 variables; if taken all levels of categorical vars into account, we have 71 variables 
...
set.seed(410)
#run the following model for mtry = 26
mtry_26 <- caret::train(price ~ ., 
                          data = traint, 
                          method = "rf",
                          tuneGrid = bestmtry,
                          ntree = 100, 
...
varimp_20 <- varImp(mtry_26)
print(varimp_20)

test_rf26 <- testt %>%
  mutate(price_hat = predict(mtry_26, testt))

MAPE(y_pred = test_rf26$price_hat, y_true = testt$price)
RMSE(test_rf26$price_hat, testt$price)
#caret::train mtry = 26, ntree = 100, all vars
#mape 0.2561298
#rmse 383185.2

#create default mtry 
set.seed(410)
#run the following model for mtry = default
mtry_default <- caret::train(price ~ ., 
                          data = traint, 
                          method = "rf",
                         # tuneGrid = bestmtry,
                          ntree = 100, 
                          ...
                          na.action = na.omit)
test_rf <- testt %>%
  mutate(price_hat = predict(mtry_default, testt))

MAPE(y_pred = test_rf$price_hat, y_true = testt$price)
RMSE(test_rf$price_hat, testt$price)
#MAPE 0.2486943
#RMSE 383379.9
write.csv(test_rf, "test_mtry_default.csv")
varImp(mtry_default)
```
