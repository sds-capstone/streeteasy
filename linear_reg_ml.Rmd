...
library(readr)
sale_listings <- read_csv(here::here("sale_listings.csv"))
...
# I thought this is what Yipeng said to do, so I don't see why not?! -Emma
```

It suddenly came to me that we could probably use LASSO if we eventually populate many vars     Aha! I remember LASSO. Good call. -Emma
...
model.seq <- regsubsets(logPrice ~ bedrooms + 
                          bathrooms + 
                          anyrooms + 
                          logSize_sqft + 
                          time_to_subway + 
